...
import 'package:poc_agora_io/src/shared/design_system/themes/app_text_field_color.dart';
...
class PocAgoraIoTextField extends StatefulWidget {
  final String hintText;
  final PocAgoraIoTextFieldController? controller;
...
class _PocAgoraIoTextFieldState extends State<PocAgoraIoTextField> {
  ...
  void initState() {
    if (widget.controller == null) {
      controller = PocAgoraIoTextFieldController(null);
    } else {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<_TextFieldState>(
      ...
      stream: Rx.combineLatest2<PocAgoraIoTextFieldControllerState, bool,
          _TextFieldState>(
        controller.stream,
        focusSubject.stream,
  ...
  ThemeData generateTheme(
    ...
    PocAgoraIoTextFieldControllerState? state,
  ) {
    ...
    return currentTheme.copyWith(
      inputDecorationTheme: currentTheme.inputDecorationTheme.copyWith(
        prefixStyle: PocAgoraIoTextStyle.bodyText1.style.copyWith(
          color: widget.color.prefix,
        ),
        contentPadding: const EdgeInsets.symmetric(
          vertical: SpacingTokens.tera,
        ),
        hintStyle: PocAgoraIoTextStyle.headline5.style.copyWith(
          color: widget.color.hintText,
        ),
        errorStyle: PocAgoraIoTextStyle.caption.style.copyWith(
  ...
  TextStyle style(bool hasFocus) {
    if (hasFocus == true) {
      return PocAgoraIoTextStyle.headline5.style.copyWith(
...
class _TextFieldState {
  final bool hasFocus;
  final PocAgoraIoTextFieldControllerState state;
...
class _Prefix extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(
        text,
        style: PocAgoraIoTextStyle.bodyText2.style.apply(color: textColor),
      ),
    );
  }
}
